...
if (!args[0]) return message.channel.send(`${message.author}, düzgün bir şarkı adı gir. ❌`);
        ...
        if (!res || !res.tracks.length) return message.channel.send(`${message.author}, kanka bulamadım link sal. ❌`);
        ...
        embed.setTimestamp();
        embed.setFooter('Aslı ❤️ Rencia', message.author.avatarURL({ dynamic: true }));
